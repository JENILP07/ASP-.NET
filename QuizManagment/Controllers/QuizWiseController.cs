...
namespace QuizManagment.Controllers
{
   ...
   /*     public IActionResult AddForm()
        ...
        private void QuestionDropDown()
        {
            ViewBag.QuestionList = new List<QuestionDropDown>();
            string connectionString = _configuration.GetConnectionString("ConnectionString");

            using (SqlConnection connection = new SqlConnection(connectionString))
            using (SqlCommand command = new SqlCommand("PR_Question_DROPDOWN", connection))
            {
                connection.Open();
                command.CommandType = CommandType.StoredProcedure;
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    DataTable dt = new DataTable();
                    dt.Load(reader);
                    ViewBag.QuestionList = dt.AsEnumerable().Select(row => new QuestionDropDown
                    {
